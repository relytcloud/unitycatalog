...
/**
 * The bounded securable set for the Users-page summary: every catalog plus
 * every schema in every catalog (one LIST per catalog). Tables are
 * deliberately excluded here — they live behind the explicit full scan.
 */
export function useAllSecurableRefs() {
  return useQuery<SecurableRef[]>({
    ...
    queryFn: async () => {
      ...
      const refs: SecurableRef[] = catalogNames.map((name) => ({
...
async function fetchPrincipalPrivileges(
  ...
  return assignments
    .filter(
      (assignment) => (assignment.principal ?? '').toLowerCase() === principal,
    )
