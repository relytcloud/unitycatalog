...
import {
  assignmentsMentionOtherPrincipal,
...
async function fetchOwnGrants(
  ...
  try {
    ...
    return {
      privileges: privilegesForPrincipal(assignments, principal),
      sawOthers: assignmentsMentionOtherPrincipal(assignments, principal),
      failed: false,
    };
  } catch {
    return { privileges: [], sawOthers: false, failed: true };
